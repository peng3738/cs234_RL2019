...
def build_mlp(mlp_input,output_size,scope,n_layers,size,output_activation=None):
  ...
  #     Input_data = tf.contrib.layers.fully_connected(Input_data, num_outputs=size, activation_fn=tf.nn.relu)
  #   if output_activation !=None:
  #     Input_data = tf.contrib.layers.fully_connected(Input_data, num_outputs= output_size, activation_fn=output_activation)
  #   else:
  #     Input_data = tf.contrib.layers.fully_connected(Input_data, num_outputs= output_size)
  #   Output=Input_data
  # return Output
...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    #print(self.discrete)
    if self.discrete:
      action_logits = build_mlp(self.observation_placeholder, self.action_dim, scope, self.config.n_layers, self.config.layer_size, output_activation=self.config.activation)
      #print("action")
      #print(action_logits)
      ...
      action_means = build_mlp(self.observation_placeholder, self.action_dim, scope, self.config.n_layers, self.config.layer_size)
      log_std = tf.get_variables("log_std", [1, self.action_dim], trainable=True)
      self.sampled_action = tf.random_normal(self.action_dim, action_means, log_std)
      self.logprob = tf.contrib.distributions.MultivariateNormalDiag(self.action_placeholder, log_std)
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    Base_loss = tf.losses.mean_squared_error(labels=self.baseline_target_placeholder, predictions=tf.squeeze(self.baseline))
    Optimizer = tf.train.AdamOptimizer(self.lr)
    self.update_baseline_op = Optimizer.minimize(Base_loss)
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for step in range(len(rewards)-2, -1, -1):
        returns[step] = self.config.gamma*returns[step+1] + rewards[step]
      #######################################################
      #########          END YOUR CODE.          ############
  ...
  def calculate_advantage(self, returns, observations):
    ...
    #########   YOUR CODE HERE - 5-10 lines.   ############
    if self.config.use_baseline:
      Basevalue = self.sess.run(self.baseline,feed_dict={self.observation_placeholder: observations})
      adv = adv-Basevalue.squeeze()
    if self.config.normalize_advantage:
      adv_mean = adv.mean()
      adv_std = adv.std()
      #adv_mean = np.mean(adv)
      #adv_std= np.sqrt(np.var(adv) / len(adv))
      adv = (adv-adv_mean)/(adv_std + 1e-12)
    #######################################################
    #########          END YOUR CODE.          ############
    return adv
