...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder("uint8", [None,state_shape[0],state_shape[1],state_shape[2]* config.state_history])
        self.a = tf.placeholder("int32", [None])
        self.r = tf.placeholder("float32", [None])
        self.sp = tf.placeholder("uint8", [None,state_shape[0],state_shape[1],state_shape[2]* config.state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        input = layers.flatten(state, scope)
        out = layers.fully_connected(input, num_actions, reuse=reuse, scope=scope, activation_fn=None)
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        update_ops = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, q_scope)
        update_target_ops = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, target_q_scope)
        qt = [tf.assign(update_target_ops[it], update_ops[it]) for it in range(len(update_ops))]
        self.update_target_op = tf.group(*qt)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_loss_op(self, q, target_q):
        ...
        Q_samp = self.r + Undone*self.config.gamma*tf.reduce_max(target_q, axis=1)
        unitMat = tf.one_hot(self.a, num_actions)
        q_sa = tf.reduce_sum(q*unitMat, axis=1)
        Diff = tf.squared_difference(Q_samp, q_sa)
    ...
    def add_optimizer_op(self, scope):
        ...
        if self.config.grad_clip:
            clipped_grads_and_vars = [(tf.clip_by_norm(grad_var[0], self.config.clip_val), grad_var[1]) for grad_var in grads_and_vars]
        #print(clipped_grads_and_vars)
        self.train_op = optimizer.apply_gradients(clipped_grads_and_vars)
        self.grad_norm = tf.global_norm([it[0] for it in clipped_grads_and_vars])
        ##############################################################
        ######################## END YOUR CODE #######################
